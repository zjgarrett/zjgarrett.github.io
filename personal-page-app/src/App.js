...
function App() {
 ...
 useEffect(() => {
  ...
  const updateBackground = () => {
    ...
    const scrollDistance = root.scrollTop || document.documentElement.scrollTop;
    const maxDistance = root.scrollHeight - root.clientHeight;
    const percentScrolled = Math.min(Math.log2(1 + 4* scrollDistance / maxDistance), 1);

    const percentNotScrolled = 1 - percentScrolled;
    const secondHeight = Math.max(percentNotScrolled, .1);
